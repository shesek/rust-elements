...
mod test {
    ...
    fn roundtrips() {
        ...
        for (idx, addr) in vectors.into_iter().enumerate() {
            if let Ok(roundtripped) = addr.to_string().parse() {
                let rt: Address = roundtripped;
                assert_eq!(addr.to_string(), rt.to_string());
            } else {
                assert_eq!(Ok(addr.clone()), addr.to_string().parse(), "vector #{}: {}", idx, addr);
            }
        }
    }
